...
fn main() {
    ...
    let upper_left = parse_complex(&args[3]).expect("error left corner");
    let lower_rignt = parse_complex(&args[4]).expect("error right corner");
    let mut pixels = vec![0; bounds.0*bounds.1];

    render(&mut pixels, bounds, upper_left, lower_rignt);
